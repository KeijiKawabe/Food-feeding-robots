...
def main():
    ...
    # OpenCVによるキャリブレーション
    # Eye-to-Hand: Camera -> Gripper (^cT_g) を求める
    # 出力はドキュメント通り R_cam2gripper, t_cam2gripper
    try:
    ...
    # 結果: T_cam2gripper (Camera -> Gripper の変換)
    T_cam2gripper = np.eye(4)
    T_cam2gripper[:3, :3] = R_cam2gripper
    T_cam2gripper[:3, 3] = t_cam2gripper.flatten()
    
    # Eye-to-Handの場合、実際に必要なのは Base -> Camera なので変換
    # 最後のサンプルの Base -> Gripper を使って計算
    R_base2gripper_last = R_gripper2base_list[-1].T
    t_base2gripper_last = -R_gripper2base_list[-1].T @ t_gripper2base_list[-1]
    T_base2gripper_last = rt_to_matrix(R_base2gripper_last, t_base2gripper_last)
    
    # Base -> Camera = Base -> Gripper @ Gripper -> Camera
    T_gripper2cam = np.linalg.inv(T_cam2gripper)
    T_base2cam = T_base2gripper_last @ T_gripper2cam
    
    print("\n" + "="*50)
    print("  CALIBRATION RESULT")
    print("="*50)
    print("\nT_cam2gripper (Camera -> Gripper) [メートル単位]:")
    print(np.round(T_cam2gripper, 6))
    ...
    for i in range(len(R_gripper2base_list)):
        ...
        # Base -> Camera を使ってマーカー位置を計算
        T_base2cam_i = T_base2gripper_i @ T_gripper2cam
        T_base2marker = T_base2cam_i @ T_cam2target_i
